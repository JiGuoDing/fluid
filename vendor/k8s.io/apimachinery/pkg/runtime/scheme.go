...
*/
//
// Group 是 API 对象的逻辑分组，通常将相关的资源类型组织在一起。
// Version 是 API 的一个特定迭代，随着 API 的发展，可能会引入新功能或更改现有功能，这就需要创建新的版本。
// Version 通常用一个字符串表示，如: v1, v1alpha1, v1beta1 等。
// Kind 是 API 对象的具体类型，定义了对象的行为和结构，如 Pod, Deployment, Service等，每个种类都有其特定的用途和属性。
// 这三个属性共同构成了一个唯一的标识符，用于在 API 中引用特定的资源类型，例如 apps/v1/Deployment 表示 apps 组的 v1 版本中的 Deployment资源类型。
type Scheme struct {
	...
	//用于反向查找，将Go类型映射到其对应的所有GVK，其中reflect.Type不应是指针类型
...
//
// 将一组已知的 Go 类型（对象）注册到一个 Scheme 中，并且指定这些类型属于某个特定的版本
...
//
// 将一个已知的 Go 类型（对象）注册到一个 Scheme 中，并且指定该类型在 API 中的表示形式，包括其所属的组、版本及种类
